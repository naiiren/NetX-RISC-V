...
OUT_DIR="${OUT_DIR:-${REPO_DIR}/reports/activity_iverilog_baseline_$(date +%Y%m%d_%H%M%S)}"
...
echo "[1/5] Preparing baseline RTL for Icarus"
cp "${FPGA_DIR}/rv32i_fpga.v" "${SANITIZED_DIR}/rv32i_fpga.v"
cp "${FPGA_DIR}/core.v" "${SANITIZED_DIR}/core.v"

awk '
    BEGIN { skip = 0 }
    /^module VGA_GLYPH_ROW\(/ {
        print "module VGA_GLYPH_ROW("
        print "    input  [7:0] ch,"
        print "    input  [3:0] row,"
        print "    output [7:0] bits"
        print ");"
        print "assign bits = {ch[3:0], row[3:0]} ^ {row, ch[7:4]};"
        print "endmodule"
        skip = 1
        next
    }
    skip { next }
    { print }
' "${FPGA_DIR}/verilog_periph.v" > "${SANITIZED_DIR}/verilog_periph.v"
...
echo "[2/5] Compiling whole-system baseline RTL with Icarus"
"${IVERILOG}" -g2012 -o "${SIM_BIN}" \
    "${SANITIZED_DIR}/rv32i_fpga.v" \
    "${SANITIZED_DIR}/core.v" \
    "${SANITIZED_DIR}/verilog_periph.v" \
